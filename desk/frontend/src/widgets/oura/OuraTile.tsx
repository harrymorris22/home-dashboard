...
const startConnect = () => {
  // Same-tab nav: iPad Safari (esp. in PWA mode) handles new-tab OAuth poorly.
  window.location.href = "/api/widgets/oura/oauth/start";
};

/** Oura step-count tile. Read-only. */
export function OuraTile() {
  const { data, error, isLoading } = useOura();

  // On return from OAuth (?connected=1) or error (?oauth_error=...), force
  // an SWR revalidation so the tile populates immediately, then clean the URL.
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    if (params.has("connected") || params.has("oauth_error")) {
      mutate("/api/widgets/oura/summary");
  ...
  if (error) {
    ...
    if (apiErr.status === 503 && detail.error === "oura_oauth_not_configured") {
      return (
        <Card>
          <h2 className="hud-label">Steps</h2>
          <p
            className="text-secondary text-sm mt-3"
            data-testid="oura-unconfigured"
          >
            {detail.instruction || "Set Oura credentials in Add-on options."}
          </p>
        </Card>
      );
    }

    if (apiErr.status === 503 && detail.error === "oura_not_connected") {
      return (
        <Card
          ...
          role="button"
          tabIndex={0}
          onKeyDown={(e) => (e.key === "Enter" || e.key === " ") && startConnect()}
          data-testid="oura-not-connected"
        >
          <h2 className="hud-label">Steps</h2>
          <div className="font-display text-2xl uppercase tracking-tight text-primary mt-2">
            Connect Oura →
          </div>
        </Card>
      );
    ...
    if (apiErr.status === 503 && detail.error === "oura_token_invalid") {
      return (
        <Card
          ...
          tabIndex={0}
          onKeyDown={(e) => (e.key === "Enter" || e.key === " ") && startConnect()}
          data-testid="oura-token-invalid"
        >
          <h2 className="hud-label">Steps</h2>
          <div className="font-display text-2xl uppercase tracking-tight text-primary mt-2">
            Reconnect Oura →
          </div>
          <p className="text-xs text-secondary">Token expired or revoked</p>
        </Card>
      );
    }
